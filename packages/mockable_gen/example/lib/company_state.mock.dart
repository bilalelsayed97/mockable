...
import 'package:mockable_gen_example/company_state.dart';
...
extension CompanyStateMock on CompanyState {
  static CompanyState mockInitial() => CompanyState.initial();

  static CompanyState mockLoaded() => CompanyState.loaded(
        companies: List.generate(3, (_) => _$mockCompany()),
        searchQuery: MockFaker.word(),
      );

  static CompanyState mockError() => CompanyState.error(
        message: MockFaker.sentence(),
      );

  static CompanyState mock() => mockLoaded();

  static List<CompanyState> mockList([int count = 10]) =>
      List.generate(count, (_) => CompanyStateMock.mock());
}

Company _$mockCompany() => Company(
      name: MockFaker.name(),
